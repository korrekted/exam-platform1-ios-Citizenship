...
private extension OSlideGenderView {
    ...
    func makeManView() -> OSlideGenderItemView {
        let tapGesture = UITapGestureRecognizer(target: self, action: #selector(selectedMan))
        
        let view = OSlideGenderItemView()
        view.isUserInteractionEnabled = true
        view.addGestureRecognizer(tapGesture)
        view.imageView.image = UIImage(named: "Onboarding.SlideGender.Man")
        view.translatesAutoresizingMaskIntoConstraints = false
        addSubview(view)
        return view
    }
    
    func makeWomanView() -> OSlideGenderItemView {
        let tapGesture = UITapGestureRecognizer(target: self, action: #selector(selectedWoman))
        
        let view = OSlideGenderItemView()
        view.isUserInteractionEnabled = true
        view.addGestureRecognizer(tapGesture)
        view.imageView.image = UIImage(named: "Onboarding.SlideGender.Woman")
        view.translatesAutoresizingMaskIntoConstraints = false
        addSubview(view)
        return view
    }
